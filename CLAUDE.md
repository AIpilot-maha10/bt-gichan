...
**`tools/sparring/scenarios.py`의 시나리오를 추가·수정하면 반드시 노션 카탈로그도 같이 갱신한다.**

- 페이지: **[🎯 하네스 시나리오 카탈로그](https://app.notion.com/p/3b00adbca4ca81d1a4cbfbae8076e283)** (덮어쓰기 O — 살아있는 문서)
- 갱신할 것: 셀 표에 한 줄 추가(WEZ 결과·shutout·**발견 경위**), 「변경 이력」에 날짜와 내용
- 진단 셀은 코드에도 `DiagCell(why=..., watch=...)`로 **발견 경위와 감시 지표**를 남긴다
  — 나중에 "이 셀이 왜 있는지" 모르는 일이 없어야 한다

### 스위트 3개 (목적이 다르면 섞지 않는다)

| 스위트 | 목적 | 명령 |
|---|---|---|
| `eval` | 대회 성능 예측 — 예선 실측 + 지터 | `baseline` / `compare` |
| `diag` | 약점 탐지 — 격자 전수, **어느 칸이 0점인가** | `diag` |
| `regress` | 사고 재발 방지 — 과거 실패 상황 고정 | `regress` |

> 처음엔 한 풀에 가중추첨했다가 30시드에서 `adversarial`이 1판만 나왔다(의도 15%, 실제 3%).
> 위험 시나리오가 통계에서 지워진다.

### 예선 시나리오 실측 (서버 로그 97판 전수조사, 8/3)

| 항목 | 값 |
|---|---|
| 고도 | **4,572m = 15,000ft 고정** |
| 분리거리 | **497m** |
| 헤딩 | 서로 반대(±90°), 분리축과 수직 → **빔 패스** |
| 속도 | **200.1 m/s**(46판) 또는 **118.6 m/s**(41판), 양측 동일 |

15,000ft 밀도비 √(ρ/ρ₀)=**0.7932** → CAS **309kt** / **183kt**.
**둘 다 350kt 이하 = 교범상 1서클(최소반경) 싸움이 정답**(§4.8.4.2.4.2.2).

---

## 🧪 판정 규칙 & 현재 빌드 (필수 — 8/4 확립)

### 현재 빌드: `bt-v7.2` (태그·보관본 `D:/aipilot-artifacts/dll-archive/bt-v7.2/`)

채택 8건: EP4(좌표버그) · **A-1 지표 계층 5종** · EP24/26/28(에너지 회복 강하) · **EP33~36(개전 래치)**

| 검증 | 결과 |
|---|---|
| 신규 시드(70000) btjegal | 승 20→**32**, 적체력 0.328→**0.142** |
| 외부 4상대 승 합계 | v7.1 12 vs **v7.2 17** |
| diag 12셀 | 79.20→**83.84** |
| 추락 | **0** (eval 560판·diag 180판·regress 4건) |

### ⚠️ 판정은 반드시 **5상대 로스터**로

**2상대 판정이 이 프로젝트 최대의 구조적 약점이었다.** btjegal/jegalmin 둘로만 판정해
채택했는데 v7.1과 머리맞대기를 붙이니 3승 16패였다.

```
python -m tools.sparring baseline --seeds 30 --seed-base 50000
# 기본 로스터: jegalmin,btjegal,v6,v1,v71
```

- 구버전 gichan 상대는 `Rule_gichan_legacy.xml`(A-1 이전 트리)로 되살렸다
- `coordfix` ≡ `v7.1` (좌표수정~v7.1 사이 EP가 전부 기각/죽은코드) → 로스터에서 제외
- `AIP_BASE*`는 무기력(양쪽 0승) → 스모크용

### ⛔ 이미 반증됐다 — 다시 시도하지 말 것

| 방향 | 반증 근거 |
|---|---|
| **에너지·선회로 jegalmin 뚫기** | EP8/9/17/18/19/20 실패. CAS 189→355kt 올려도 적체력 0.985→0.994 |
| **사격 경로에 개입 추가** | EP21/25/27/37 전부 실패. **발동 1.0%로도 btjegal 3판 손실** |
| **XML 포트 스윕** | 6변형 전부 현재값보다 나쁨. `GunAimAtaDeg` 22°는 국소 최적 |
| **지연추적/Ease Repo 직접 이식** | EP37 — 오버슛은 피하나 각도를 더 잃는다 |
| **조준 중 강하 억제** | EP38/39 — 머리맞대기는 개선되나 외부 상대 손실(트레이드오프) |

### 🔍 jegalmin전 병목 (규명 완료)

**적이 Nz 6.03G 브레이크로 종말 추적을 깬다.** 우리 추적은 오히려 더 안정적이다(부호반전 절반).
확보는 되는데(진입 4.2회 > btjegal 3.6회) **유지가 3.5배 짧다**(5.81s vs 20.13s).
→ 파라미터가 아니라 **새 상태(Ease/Repo) 설계**가 필요하다.

### 판정 대시보드 순서

`tgt_health_final` → `승` → `wez_dealt_ticks_phase1only` → (참고만) shutout

⚠️ **shutout을 헤드라인으로 쓰지 말 것.** 하네스는 Phase2/3 넓은 콘까지 세는데
로컬 sim 데미지는 **Phase1 콘(1°) 고정**이라 어긋난다.
...
| **중립 머지에서 우위 전환 실패** | **AETCTTP §4.8 (HABFM)** | ✅ (8/3) |
| 수직기동·요요 세부 설계 | Shaw *Fighter Combat* | ⬜ 미조사 |
| 1v1 넘는 기동 조합 | AETCTTP §4.9 (ACM) | ⬜ 미조사 |
| 저속·고AOA 조종성 | F-16 비행교범 | ⬜ 미조사 |

### 🔵 1서클 vs 2서클 (§4.8 — 8/3 조사, 예선 직결)

**중립 패스에서는 마지막에 도는 쪽이 싸움을 정한다.** 우유부단이 최악이다.

| 조건 | 강제할 싸움 |
|---|---|
| **CAS ≤ 350kt** | **1서클 (최소반경)** — 리드턴으로 동체 정렬 |
| CAS > 350kt 또는 기수 15°↓ | 2서클 (레이트) — LV를 수평선 아래로 |

- **예선 두 속도(CAS 309kt·183kt) 모두 1서클이 정답.** HardTurn 74~79%는 2서클 행동이다
- 반경: 350~250kt는 거의 일정, **250kt 미만은 반경이 다시 커진다**(§4.8.4.2.3.1)
- **적이 마지막에 도는 방향이 분기**(§4.8.3.3): 나를 향해 돌면 **2서클**, 등지고 돌면 **1서클**
- **리드턴 단서 = 급격한 후방 LOSR 증가**. 가장 흔한 실수는 **늦은 리드턴**. 의심되면 일찍(§4.8.3.1)
- 리드턴이 너무 이르면 LOSR이 얼어붙는다 → **G를 잠깐 풀어** 다시 증가시킨다(§4.8.3.2)
- **TR 목표 = 선회직경 1개**(T-38 기준 6,000ft). 적이 TR을 뺏기 시작하면 **더 벌지 말고 가진 걸 써라**(§4.8.1.3)
- 머지 목표: **적 CZ를 통과**하되 적 선회반경 **안으로 들어가면 반전 기회를 준다**(§4.8.2)
- **TCX는 이기고 있을 때만.** 승리 단서 = 후드 쪽 LOSR + AA<90°. AA가 90° 넘어가면 TCX 말고 **리드턴 한 번 더**(§4.8.4.1.5.1)
- 2서클은 원래 오래 걸린다 — **500°~1,000° 선회**가 정상(§4.8.4.1.6)
