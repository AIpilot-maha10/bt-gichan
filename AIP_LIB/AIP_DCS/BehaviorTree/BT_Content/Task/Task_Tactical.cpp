...
NodeStatus Action::Task_Tactical::tick()
{
	...
	else if (behavior == "HardTurn")
	{
		...
		// ── (EP27) 사거리 밖 고ATA에서는 **리드를 준다** ─────────────────────
		// 위 주석의 "리드 없음 = 최단 수렴"은 틀렸다. **움직이는 표적에 순수추적은
		// 최단 수렴이 아니라 꼬리 추격 곡선**이다. 교범이 순수추적을 요구하는 건
		// WEZ 조준 국면뿐이고(리드각이 LOS 콘을 오염시키므로), 사거리 밖 선회에는
		// 해당하지 않는다. 오히려 §4.8.3.1은 **리드턴**을 요구한다.
		//
		// 실측 근거 (diag_gusage, EP26 적용본, vs jegalmin):
		//   ATA 분포     0-10도  9.7%   /  90-180도 **46.6%**   (btjegal은 48.2% / 19.5%)
		//   ATA>60도(교전의 66.5%)에서의 Nz
		//     나  중앙 3.87  p90 6.36  >6G 14.2%   CAS 319kt
		//     적  중앙 5.64  p90 7.85  >6G **46.1%**
		//   p90이 6.36이니 **당길 능력은 있는데 지속을 못 한다.**
		//   에너지는 EP24/EP26으로 확보했는데(CAS 코너 도달) 각도로 전환이 안 된다
		//   = 교범 §4.2.3 "Energy versus Nose Position" 그 자체다.
		//
		// 안전장치: **사거리 근처에서는 절대 리드를 주지 않는다.** 1도 콘 조준은
		//   적 현재위치를 겨눠야 하고, EP21/EP25가 사격 국면을 건드려 btjegal 승리를
		//   깎아먹은 전례가 있다. 여기서는 wez.rMax*1.5 밖에서만 발동한다.
		//
		// ⚠️ (EP27 결론) **기각. 발동 차단.**
		//   시드 50000  btjegal 적체력 0.156->0.262 | 승 30->**25** | jegalmin shutout 13->**7**
		//   시드 10000  btjegal 적체력 0.285->0.314 | 승 18->**15** | jegalmin shutout  9->10
		//
		//   jegalmin shutout 13->7은 큰 개선이지만 **승리로 전환되지 않는다**
		//   (승 0->0, 적체력 0.994->0.985). 반면 btjegal에서 잃는 5+3판은 실제 승리다.
		//   나쁜 교환이다.
		//
		//   ★ 세 번째 같은 패턴이다. EP21(스로틀)·EP25(스로틀)·EP27(리드)은 전부
		//     **btjegal의 사격 기하에 닿는 것**을 건드려 실패했다. 이번엔 사거리
		//     1.5배 밖으로 게이트를 걸었는데도 샜다 — 사거리 밖의 선회 궤적이
		//     그대로 진입 각도를 만들기 때문에 "사거리 밖"이라고 안전하지 않다.
		//
		//   교훈: **btjegal전 27~30승은 잘 맞춰진 균형이고, 그 경로에 닿는 변경은
		//     거의 항상 손해다.** jegalmin전만 겨냥하려면 btjegal전에서 절대 발동하지
		//     않는 조건(예: 고도·CAS 기반. EP24/EP26이 그렇게 성공했다)을 써야 한다.
		if (false && dist > wez.rMax * 1.5f && los > 60.0f)
		{
			// ATA가 클수록 리드를 키운다 (60도=0, 120도 이상=최대)
			const float k = clampf((los - 60.0f) / 60.0f, 0.0f, 1.0f);
			const float leadSec = k * 1.5f;
			vp = tgtPos + bb->TargetForwardVector * (bb->TargetSpeed_MS * leadSec);
			aiming = false;   // 조준 국면이 아니다 — 각도를 버는 중이다
		}
