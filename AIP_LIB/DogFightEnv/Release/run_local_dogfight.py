...
def parse_args():
    ...
    parser.add_argument("--scenario", choices=["default", "preliminary"], default="default",
                        help="preliminary = 예선형 근접 측면배치(2000~3000ft) + 다른 방향.")
    parser.add_argument("--deep-log", action="store_true",
                        help="JSBSim 심층 텔레메트리(연료/G/마하/받음각/에너지)를 CSV로 기록.")
    return parser.parse_args()


# JSBSim 51-state 인덱스 (FighterSim._state)
_STATE_IDX = {
    "n": 0, "e": 1, "d": 2, "roll": 3, "pitch": 4, "yaw": 5,
    "cas": 12, "aoa": 13, "aos": 14, "fuel": 23, "tas": 27, "mach": 29,
    "vert_vel": 30, "nz": 31, "ny": 32, "simtime": 41, "alt": 44, "health": 45,
}


def _deep_log_row(step, own, tgt):
    import math as _m
    def g(s, k):
        i = _STATE_IDX[k]
        return float(s[i]) if s is not None and len(s) > i else 0.0
    own_alt = -g(own, "d")
    tgt_alt = -g(tgt, "d")
    own_spd = g(own, "tas")
    energy = own_alt + (own_spd * own_spd) / (2.0 * 9.81)
    return {
        "step": step,
        "sim_time": round(g(own, "simtime"), 2),
        "own_alt": round(own_alt, 1), "own_tas": round(own_spd, 1),
        "own_cas": round(g(own, "cas"), 1), "own_mach": round(g(own, "mach"), 3),
        "own_aoa": round(g(own, "aoa"), 2), "own_nz": round(g(own, "nz"), 2),
        "own_ny": round(g(own, "ny"), 2), "own_fuel_lbs": round(g(own, "fuel"), 1),
        "own_vert_vel": round(g(own, "vert_vel"), 1), "own_energy_m": round(energy, 0),
        "own_health": round(g(own, "health"), 3),
        "enemy_alt": round(tgt_alt, 1), "enemy_tas": round(g(tgt, "tas"), 1),
        "enemy_health": round(g(tgt, "health"), 3),
    }
...
def main():
    ...
    if args.scenario == "preliminary":
        # 예선형: East축 2000~3000ft 측면배치 + 서로 다른 방향(정면 아님)
        env_config["initial_scenario"] = {
            "mode": "preliminary",
            "separation_ft_range": [2000.0, 3000.0],
            "altitude_m": 3000.0,
            "heading_offset_deg": 180.0,
            "speed_mps_range": [200.0, 250.0],
        }
    ...
    with activate_rule_xml(args.bt_rule_xml, ROOT):
        ...
        try:
            ...
            while not (terminated or truncated):
                ...
                step_i += 1
                if args.deep_log:
                    deep_rows.append(_deep_log_row(
                        step_i,
                        getattr(env, "_ownship_state", None),
                        getattr(env, "_target_state", None),
                    ))

            if args.deep_log and deep_rows:
                import csv as _csv
                import time as _time
                out_dir = ROOT / "engagement_logs"
                out_dir.mkdir(parents=True, exist_ok=True)
                path = out_dir / f"deep_local_{_time.strftime('%Y%m%d_%H%M%S')}.csv"
                with open(path, "w", newline="", encoding="utf-8") as f:
                    w = _csv.DictWriter(f, fieldnames=list(deep_rows[0].keys()))
                    w.writeheader()
                    w.writerows(deep_rows)
                print(f"[DeepLog] {len(deep_rows)} rows -> {path}")
