...
class DogFightEnv(gym.Env):
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        elif scenario_mode == "preliminary":
            self._apply_preliminary_initial_scenario(scenario)
        else:
    ...
    def _apply_preliminary_initial_scenario(self, scenario: dict) -> None:
        """예선 시나리오: 두 기체를 측면(East축)으로 2000~3000ft 떨어뜨려 나란히 배치하고
        서로 다른 방향을 보게 한다(정면 head-on이 아님 → 적 획득용 turn-in/SEARCH 필요).
        config 예: {"mode":"preliminary","separation_ft_range":[2000,3000],
                    "altitude_m":3000,"heading_offset_deg":180,"speed_mps_range":[200,250]}"""
        sep_min_ft, sep_max_ft = self._range_values(
            scenario.get("separation_ft_range", [2000.0, 3000.0])
        )
        separation_m = float(self.np_random.uniform(sep_min_ft, sep_max_ft)) * FEET_TO_METER
        altitude_m = float(scenario.get("altitude_m", 3000.0))
        center_n = float(scenario.get("center_n_m", 0.0))
        center_e = float(scenario.get("center_e_m", 0.0))
        speed_min, speed_max = self._range_values(
            scenario.get("speed_mps_range", [200.0, 250.0])
        )
        # own은 북(0도), target은 heading_offset만큼 다른 방향(기본 180 = 반대)
        base_heading = float(scenario.get("base_heading_deg", 0.0))
        heading_offset = float(scenario.get("heading_offset_deg", 180.0))
        own_heading = self._wrap_heading(base_heading)
        target_heading = self._wrap_heading(base_heading + heading_offset)
        own_speed = float(self.np_random.uniform(speed_min, speed_max))
        target_speed = float(self.np_random.uniform(speed_min, speed_max))
        half_sep = separation_m / 2.0

        # East축으로 분리(나란히), 헤딩은 분리축과 무관하게 서로 다른 방향
        self.change_init_position(
            "ownship",
            init_n=center_n,
            init_e=center_e - half_sep,
            init_d=-altitude_m,
            init_roll=0.0,
            init_pitch=0.0,
            init_heading=own_heading,
