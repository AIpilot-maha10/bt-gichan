...
class AIPilot:
    def __init__(self, filename: str = "AIP_DCS_ownship.dll"):
        ...
        # 신규 export(gichan DLL 전용). jegalmin 등 구버전 DLL엔 없을 수 있어 방어적으로 바인딩.
        self._has_set_rule_xml = hasattr(self.AIPilotDLL, "SetRuleXmlPath")
        if self._has_set_rule_xml:
            self.AIPilotDLL.SetRuleXmlPath.argtypes = [ct.c_char_p]
            self.AIPilotDLL.SetRuleXmlPath.restype = None
        self._has_task_name = hasattr(self.AIPilotDLL, "GetCurrentTaskName")
    ...
    def SetRuleXmlPath(self, path: str) -> bool:
        """로드할 Rule XML 경로 지정. CreateBehaviorTree 전에 호출. 구버전 DLL이면 무시(False)."""
        if not self._has_set_rule_xml or not path:
            return False
        self.AIPilotDLL.SetRuleXmlPath(str(path).encode("utf-8"))
        self._debug_log(f"SetRuleXmlPath path={path}")
        return True

    def GetCurrentTaskName(self, my_id) -> str:
        """현재 실행 중인 Task(전략) 이름. 구버전 DLL이면 빈 문자열."""
        if not self._has_task_name:
            return ""
        raw = self.AIPilotDLL.GetCurrentTaskName(int(my_id))
        if not raw:
            return ""
