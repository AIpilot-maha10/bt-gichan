...
from dogfight.tools.engagement_monitor import EngagementMonitor, EngagementPolicy
...
def parse_args():
    ...
    parser.add_argument("--engage-log", action="store_true", help="Record per-tick engagement data to CSV and print post-fight summary.")
    parser.add_argument("--engage-log-dir", default="engagement_logs", help="Directory for engagement CSV logs.")
    parser.add_argument("--engage-print-interval", type=int, default=60, help="Print live status every N ticks.")
    ...
    with activate_rule_xml(args.bt_rule_xml, ROOT):
        ...
        if args.engage_log:
            monitor = EngagementMonitor(
                ...
                log_dir=args.engage_log_dir,
                print_interval=args.engage_print_interval,
            )
            command_policy = EngagementPolicy(command_policy, monitor)
