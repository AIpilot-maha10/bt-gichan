...
**개전 0.5초 고도차 래치(EP33~36)가 서버에서도 성립하는가.**

로컬 IC는 결정론적이라 개전 시 양측이 정확히 같은 고도(4,702m)지만,
서버는 접속 타이밍이 달라 개전 시점 상태가 다를 수 있다.
래치가 잘못 "비대칭"으로 잡히면 에너지 회복 강하가 꺼져 EP24/26 이득을 잃는다.

확인법: `--engage-log`로 교전 로그를 받아 개전 직후 양측 고도차를 본다.
300m 이상 벌어져 있으면 래치 임계값(`LATCH_ALT_M`) 재조정이 필요하다.
